...
export const PictureMatchActivity: React.FC<PictureMatchActivityProps> = () => {
  ...
  const targetChar = currentQuestion.targetCharacter.char;
  const targetWord = currentQuestion.targetWord.word;
  ...
  return (
    <div className="min-h-screen bg-toy-canvas flex flex-col justify-between">
      ...
      <main className="flex-1 flex flex-col items-center justify-center max-w-4xl w-full mx-auto px-4 py-4">
        ...
        {/* Target Character Spotlight & Audio Trigger */}
        <div className="flex flex-col items-center my-3">
          <div className="flex items-center gap-4 bg-white px-6 py-3 rounded-3xl border-4 border-toy-yellow shadow-toy-md">
            <span className="text-5xl md:text-6xl font-black font-hindi text-toy-orange-dark">
              {targetChar}
            </span>
        ...
        {/* Dynamic Prompt / Feedback Banner */}
        <FeedbackBanner
          feedback={feedback}
          attempts={attempts}
          promptText={`कौन सा शब्द "${targetChar}" से शुरू होता है?`}
          correctMessage={`बहुत बढ़िया! 🎉 ${targetChar} → ${targetWord}`}
        />

        {/* 3 Image/Word Option Cards */}
        <div className="w-full grid grid-cols-3 gap-3 md:gap-6 my-4 max-w-3xl">
