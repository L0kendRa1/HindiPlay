...
import { SentenceDifficulty, SENTENCE_DIFFICULTIES } from '../types/sentenceBuilder';
import { Star } from 'lucide-react';

interface SentenceDifficultySelectorProps {
  currentDifficulty: SentenceDifficulty;
  onSelectDifficulty: (difficulty: SentenceDifficulty) => void;
  disabled?: boolean;
}

export const SentenceDifficultySelector: React.FC<SentenceDifficultySelectorProps> = ({
  currentDifficulty,
  onSelectDifficulty,
  disabled = false,
}) => {
  const difficulties: SentenceDifficulty[] = ['easy', 'medium', 'hard'];
  ...
  return (
    <div className="flex items-center justify-center gap-2 bg-white/80 backdrop-blur-xs p-1.5 rounded-2xl border-2 border-slate-200 shadow-toy-sm max-w-lg mx-auto mb-3">
      {difficulties.map((diffKey) => {
        const config = SENTENCE_DIFFICULTIES[diffKey];
        ...
        return (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl font-black font-hindi text-xs md:text-sm transition-all duration-200 ${
              isActive
                ? 'bg-gradient-to-r from-toy-purple to-pink-500 text-white shadow-toy-sm border-2 border-purple-700 scale-105'
